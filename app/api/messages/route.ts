...
export async function POST(request: Request) {
  try {
    ...
    console.log("Message request received:", { chatId, tutorId, contentLength: content?.length, rerun })

    if (!chatId || !content || !tutorId) {
      return NextResponse.json({ error: "chatId, content, and tutorId are required" }, { status: 400 })
    }
    ...
    await db.collection("chats").updateOne({ _id: new ObjectId(chatId) }, { $set: { updatedAt: now } })

    // Get previous messages for context
    const previousMessages = await db.collection("messages").find({ chatId }).sort({ createdAt: 1 }).toArray()
    ...
    try {
      ...
      const stream =  (response, {
        async onCompletion(completion) {
          ...
          const result = await db.collection("messages").insertOne(assistantMessage)
          console.log("Saved assistant message with ID:", result.insertedId)
          ...
          if (previousMessages.length <= 1) {
            try {
              ...
              await db.collection("chats").updateOne({ _id: new ObjectId(chatId) }, { $set: { title } })
      ...
      const fallbackResult = await db.collection("messages").insertOne(fallbackMessage)
