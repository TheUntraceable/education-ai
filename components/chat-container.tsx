...
import { Send, ArrowUp, Trash2, Menu, GraduationCap, MessageSquare, PlusCircle } from "lucide-react"
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      while (true) {
        ...
        setMessages((prev) =>
          prev.map((msg) => (msg._id === streamingId ? { ...msg, content: accumulatedContent } : msg)),
        )
      }
      ...
      setMessages((prev) => prev.filter((msg) => !msg._id.startsWith("temp-") && !msg._id.startsWith("streaming-")))
    } finally {
      setLoading(false)
      setStreamingContent("")
    }
          ...
          <div className="space-y-6">
            {messages.map((message, index) => (
              <MessageItem
                ...
                isLastUserMessage={
                  message.role === "user" && index === messages.findLastIndex((m) => m.role === "user")
                }
                isStreaming={message._id.startsWith("streaming-")}
              />
            ))}
            {loading && !messages.some((msg) => msg._id.startsWith("streaming-")) && (
        ...
        <div className="border-t p-4">
          <form onSubmit={handleSubmit} className="flex items-end gap-2">
            ...
            <Button type="submit" size="icon" disabled={!input.trim() || loading}>
              {editingMessage ? <ArrowUp className="h-4 w-4" /> : <Send className="h-4 w-4" />}
            </Button>
          </form>
